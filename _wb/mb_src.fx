...
////////////////////////////////////////
float get_fog_amount_native(float d)
{
...
VS_OUTPUT_FONT_X vs_main_no_shadow(uniform const bool isSarlacc, float4 vPosition : POSITION, float3 vNormal : NORMAL, float2 tc : TEXCOORD0, float4 vColor : COLOR0, float4 vLightColor : COLOR1)
{
...
PS_OUTPUT ps_main_no_shadow(VS_OUTPUT_FONT_X In, uniform const bool isgalaxy=false) 
{
	PS_OUTPUT Output;
...
VS_OUTPUT_SPECULAR_ALPHA vs_specular_alpha (uniform const int PcfMode, float4 vPosition : POSITION, float3 vNormal : NORMAL, float2 tc : TEXCOORD0, float4 vColor : COLOR0, uniform const bool swytraffic = false)
{
  ...
  if(swytraffic){
	...
	Out.Fog = get_fog_amount_new(d,u);
...
VS_OUTPUT_CRAP vs_swconquest_hologram(uniform const bool isAnimated, float4 vPosition : POSITION, float3 vNormal : NORMAL, float2 tc : TEXCOORD0, float4 vColor : COLOR0)
{
	VS_OUTPUT_CRAP Out = (VS_OUTPUT_CRAP)0;
...
PS_OUTPUT ps_swconquest_hologram(PS_INPUT_CRAP In)
{
	PS_OUTPUT Output;
...
VS_OUTPUT_CRAP vs_swconquest_lightsaber(float4 vPosition : POSITION, float3 vNormal : NORMAL, float2 tc : TEXCOORD0, float4 vColor : COLOR0)
{
	VS_OUTPUT_CRAP Out = (VS_OUTPUT_CRAP)0;
...
PS_OUTPUT ps_swconquest_lightsaber(PS_INPUT_CRAP In)
{
	PS_OUTPUT Output;
...
PS_OUTPUT ps_main(VS_OUTPUT In, uniform const int PcfMode, uniform const bool isGlowEnabled = false, uniform const bool isLavaEnabled = false)
{
	PS_OUTPUT Output;
